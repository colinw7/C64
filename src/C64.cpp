...
//cpu->memset(0x73, &kernel_data[0x3A2], 0x17);
  ...
  cpu->setByte(0xD000, 0x00); // Sprite Pos (0xD000 - 0xD010)
  cpu->setByte(0xD011, 0x9B); // SCROLY (high bit of raster compare, 25 row)
  cpu->setByte(0xD012, 0x00); // RASTER
  cpu->setByte(0xD013, 0x00); // LPENX
  cpu->setByte(0xD014, 0x00); // LPENY
  cpu->setByte(0xD015, 0x00); // SPENA
  cpu->setByte(0xD016, 0x08); // SCROLX
  cpu->setByte(0xD017, 0x00); // YXPAND
  cpu->setByte(0xD018, 0x14); // VMCSB
  cpu->setByte(0xD019, 0x15); // VICIRQ
  cpu->setByte(0xD01A, 0x00); // IRQMASK
  cpu->setByte(0xD01B, 0x00); // SPBGPR
  cpu->setByte(0xD01C, 0x00); // SPMC
  cpu->setByte(0xD01D, 0x00); // XXPAND
  cpu->setByte(0xD01E, 0x00); // SPSPCL
  cpu->setByte(0xD01F, 0x00); // SPBGCL
  cpu->setByte(0xD020, 0x0E); // EXTCOL
  cpu->setByte(0xD021, 0x06); // BGCOL0
  cpu->setByte(0xD022, 0x01); // BGCOL1
  cpu->setByte(0xD023, 0x02); // BGCOL2
  cpu->setByte(0xD024, 0x03); // BGCOL3
  cpu->setByte(0xD025, 0x00); // SPMC0
  cpu->setByte(0xD025, 0x00); // SPMC0
  cpu->setByte(0xD027, 0x01); // SP0COL
  cpu->setByte(0xD028, 0x02); // SP1COL
  cpu->setByte(0xD029, 0x03); // SP2COL
  cpu->setByte(0xD02A, 0x04); // SP3COL
  cpu->setByte(0xD02B, 0x05); // SP4COL
  cpu->setByte(0xD02C, 0x06); // SP5COL
  cpu->setByte(0xD02D, 0x07); // SP6COL
  cpu->setByte(0xD02E, 0x0C); // SP7COL

  cpu->setByte(0xD02F, 0x00); // Location Range: (0xD02F - 0xD03F)

  // VIC-II Register Images
  // TODO Read always 0xFF
  cpu->setByte(0xD040, 0x00); // // (0xD040 - 0xD3FF)
...
drawScreen()
{
  ...
  uchar SCROLY = cpu->getByte(0xD011);
  uchar SCROLX = cpu->getByte(0xD016);
...
//bool videoReset     = (SCROLX & 0x20);
  ...
  uchar VMCSB = cpu->getByte(0xD018);

//uchar CI2PRA  = cpu->getByte(0xDD00);
//uchar VICBank = (3 - (CI2PRA & 0x3));

//uchar IRQMASK = cpu->getByte(0xD01A);
...
//bool lightPenIRQ        = (IRQMASK & 0x08);
  ...
  if (! bitmapGraphics) {
    ...
    uchar BGCOL0 = cpu->getByte(0xD021);
    uchar COLOR  = cpu->getByte(0x286);
...
//uchar EXTCOL = cpu->getByte(0xD020);
...
//ushort charRom  = cpu->getWord(0xC000) + i*32;
  ...
  if      (! multicolor && ! extendedColor) {
    ...
    for (int cy = 0; cy < cw; ++cy, ++y, ++iy) {
      ...
      for (int cx = 0; cx < ch; ++cx, ++charRom) {
        uchar c1 = cpu->getByte(charRom + c);
    ...
    uchar BGCOL1 = cpu->getByte(0xD022);
    uchar BGCOL2 = cpu->getByte(0xD023);
    ...
    uchar BGCOL1 = cpu->getByte(0xD022);
    uchar BGCOL2 = cpu->getByte(0xD023);
    uchar BGCOL3 = cpu->getByte(0xD024);
...
drawSprites()
{
  ...
  uchar SPENA  = cpu->getByte(0xD015);
  uchar YXPAND = cpu->getByte(0xD017);
  uchar VMCSB  = cpu->getByte(0xD018);
//uchar SPBGPR = cpu->getByte(0xD01B);
  uchar SPMC   = cpu->getByte(0xD01C);
  uchar XXPAND = cpu->getByte(0xD01D);
...
drawSprite(int s, uchar addr, bool doubleWidth, bool doubleHeight, bool multicolor)
{
  ...
  uchar x = cpu->getByte(0xD000 + 2*s);
  uchar y = cpu->getByte(0xD001 + 2*s);

  uchar pc = cpu->getByte(0xD027 + s); // SP<n>COL

  uchar pc1 = cpu->getByte(0xD025    ); // SPMC0
  uchar pc2 = cpu->getByte(0xD025 + 1); // SPMC0 + 1

  ushort epos = cpu->getByte(0xD010 + s);
